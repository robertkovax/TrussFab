...
const schedulingElement = d3.select('#scheduling').node();
const schedulingElementHeight = schedulingElement.getBoundingClientRect().height;
const schedulingElementWidth = schedulingElement.getBoundingClientRect().width;
...
let dragLine =
    d3.drag()
        .on('start',
            function(d, i) {
              ...
              dots.forEach(function(dot) {
                var distance = distanceBetweenTwoPoints(event.x, event.y,
                                                        x(dot[0]), y(dot[1]));
                if (event.x < x(dot[0])) {
        ...
        .on('drag',
            function(d, i) {
              var new_x_left = movePoint(closestLeft.point,
                                         x(closestLeft.point[0]) + d3.event.dx,
                                         0, closestLeft.point[3]);
              ...
              dots[d3.select(this).attr('id')]
                  ...
                  .attr('cy', y(closestLeft.point[1]));

              var new_x_right = movePoint(
                  closestRight.point, x(closestRight.point[0]) + d3.event.dx, 0,
                  closestRight.point[3]);
              ...
              dots[d3.select(this).attr('id')]
                  ...
                  .attr('cy', y(closestRight.point[1]));
...
let scrub = d3.drag().on('drag', function(d, i) {
...
var line = d3.line().x(function(d) { return x(d[0]); }).y(function(d) {
  return y(d[1]);
});
...
var inter = setInterval(function() { updateData(); }, 10);
...
function pause_unpause() { paused = !paused; }
