  ...
  def self.node_to_scad(path, node)
    ...
    node.incidents.each do |incident|
      ...
      if incident.first_node == node
        info['addon_array'] << "[#{incident.first_elongation_length.to_l.to_s.sub('mm', '').sub('~ ', '')}, \"#{incident.second_node.id}\"0] "
        info['type_array'] << '\"SNAP\"'
      else
        vector.reverse!
        info['addon_array'] << "[#{incident.second_elongation_length.to_l.to_s.sub('mm', '').sub('~ ', '')}, \"#{incident.first_node.id}\"0] "
        info['type_array'] << '\"SNAP\"'
      end
      info['vector_array'] << vector.to_s.sub('(','[').sub(')',']')
    end

    node.pods.each do |pod|
      info['addon_array'] << '[(45 - 0 - 10), \"STAND\",0,24,10,60,0] '
      info['type_array'] << '\"STAND\"'
      info['vector_array'] << pod.direction.normalize.to_s.sub('(','[').sub(')',']')
    end
