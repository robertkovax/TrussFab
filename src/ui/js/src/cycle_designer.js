...
const tabBarHeight =
    d3.select('#actuators-tab').node().getBoundingClientRect().height;
const bodyHeight = d3.select('body').node().getBoundingClientRect().height;

const schedulingElement = d3.select('#scheduling');
const schedulingElementHeight =
    bodyHeight - tabBarHeight - 6; // magic number 4, padding?
const schedulingElementWidth =
    schedulingElement.node().getBoundingClientRect().width;

const svg = schedulingElement.append("svg")
...
var line = d3.line().x(function(d) { return x(d.x); }).y(function(d) {
  return y(d.y);
});
...
function getCircleWithID(id, selection) {
  return selection.filter(function(circle, i) { return circle.id == id; });
}
...
function pauseUnpauseCycle() { paused = !paused; }
...
function brushed() {
  ...
  dot.classed('selected',
              function(d) { return extent[0] <= d.x && d.x <= extent[1]; });
}
...
function dragLineStarted(d, i) {
  ...
  dots.forEach(function(dot) {
    var distance =
        distanceBetweenTwoPoints(coords[0], coords[1], x(dot.x), y(dot.y));
    if (coords[0] < x(dot.x)) {
...
function draggingLine(d, i) {
  var new_pos_left = movePoint(
      closestLeft.point, x(closestLeft.point.x) + d3.event.dx,
      y(closestLeft.point.y) + d3.event.dy, closestLeft.point.piston_id);
  ...
  dots[d3.select(this).attr('id')]
      .filter(function(d, i) { return i === closestLeft.point.id - 1; })
      .attr('cx', x(new_x_left))
      .attr('cy', y(new_y_left));

  var new_pos_right = movePoint(
      closestRight.point, x(closestRight.point.x) + d3.event.dx,
      y(closestRight.point.y) + d3.event.dy, closestRight.point.piston_id);
  ...
  dots[d3.select(this).attr('id')]
      .filter(function(d, i) { return i === closestRight.point.id - 1; })
...
function dragDotStarted(d) { d3.select(this).raise().classed('active', true); }
...
function dragDotEnded(d) { d3.select(this).classed('active', false); }
...
function expand(id) { sketchup.expand_actuator(id); }

function retract(id) { sketchup.retract_actuator(id); }
